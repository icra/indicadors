...
let modul_scimago={
  ...
  scimagojr_years_not_available:new Set(),

  /*methods*/

  //carregar csvs scimago disponibles (descarregats manualment)
  init(path_to_csvs){
    path_to_csvs = path_to_csvs || "csv";
    ...
    for(let y=year_inici; y<=year_final; y++){
      this.fetch_scimagocsv(y, path_to_csvs);
    }
  },
  ...
  busca_a_scimagocsv(nom_revista, issn, eIssn, year){
    if(!year) throw new Error("year not defined");

    //dictionary to search:
    let csv = this.scimagojr[year];

    //if not exists, search most recent
    if(!csv){
      let max_year = Math.max(...Object.keys(this.scimagojr));
      max_year = String(max_year);
      csv = this.scimagojr[max_year];
    }
    if(!csv) return;
